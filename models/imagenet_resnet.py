...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, decoupler=False):
        ...
        if self.decoupler:
            self.linear1 = nn.Linear(512 * block.expansion, 512 * block.expansion, bias=False)
            self.linear2 = nn.Linear(512 * block.expansion, 512 * block.expansion, bias=False)
            self.linear1.weight = torch.nn.parameter.Parameter(torch.eye(512 * block.expansion))
            self.linear2.weight = torch.nn.parameter.Parameter(torch.eye(512 * block.expansion))
    ...
    def _forward(self, x, with_latent=False, fake_relu=False, no_relu=False, logit=False):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        if self.decoupler:
            out = self.linear1(pre_out)
            if self.training or logit:
                logit_1 = self.fc(out)
                out2 = self.linear2(pre_out.detach())#
