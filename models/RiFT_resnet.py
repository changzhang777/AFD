...
class ResNet_FS(nn.Module):
    def __init__(self, input_size, block, num_blocks, num_classes=10, dropout=0.0):
        ...
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        size = 512 * block.expansion * ((input_size // 32) ** 2)
        ...
        self.linear1.weight = torch.nn.parameter.Parameter(torch.eye(size))
        self.linear2.weight = torch.nn.parameter.Parameter(torch.eye(size))
...
def ResNet18_FS(input_size, num_classes):
    return ResNet_FS(input_size, BasicBlock, [2, 2, 2, 2], num_classes=num_classes)
...
def ResNet50_FS(input_size, num_classes):
    return ResNet_FS(input_size, Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
