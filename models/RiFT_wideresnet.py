...
class WideResNet(nn.Module):
    def __init__(self, image_size=32, depth=34, num_classes=10, widen_factor=10, dropRate=0.0, normalize=False, activation='ReLU', softplus_beta=1):
        ...
        if self.normalize:
            self.fc = nn.Linear(nChannels[3] * (image_size // 32) ** 2, num_classes, bias = False)
        else:
            self.fc = nn.Linear(nChannels[3] * (image_size // 32) ** 2, num_classes)
        self.nChannels = nChannels[3] * (image_size // 32) ** 2
...
class WideResNet_FS(nn.Module):
    def __init__(self, image_size=32, depth=34, num_classes=10, widen_factor=10, dropRate=0.0, normalize=False, activation='ReLU', softplus_beta=1):
        super(WideResNet_FS, self).__init__()
        nChannels = [16, 16 * widen_factor, 32 * widen_factor, 64 * widen_factor]
        assert ((depth - 4) % 6 == 0)
        n = (depth - 4) / 6
        block = BasicBlock
        ...
        self.fc1 = nn.Linear(self.nChannels, self.nChannels, bias=False)
        self.fc2 = nn.Linear(self.nChannels, self.nChannels, bias=False)
        self.fc1.weight = torch.nn.parameter.Parameter(torch.eye(self.nChannels))
        self.fc2.weight = torch.nn.parameter.Parameter(torch.eye(self.nChannels))

    def forward(self, x, logit=False, prejection=False):
        out = self.conv1(x)
        out = self.block1(out)
        out = self.block2(out)
        out = self.block3(out)
        ...
        #             module.weight.data = F.normalize(module.weight, p=2, dim=1)
        # return self.fc(out)
        out_1 = out.view(-1, self.nChannels)
        out = self.fc1(out_1)
        if self.training or logit:
            logit_1 = self.fc(out)
            out2 = self.fc2(out_1.detach())  #
            logit_2 = self.fc(out2)
