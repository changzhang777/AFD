...
batch_size = 256
path = '/data/ZNY/feature_purification/checkpoint/ours/ResNet18-CIFAR10/pretrained=ResNet18-CIFAR10/newest/2023-11-30 17:12:13.404644-alpha-0.1-sigma-0.1-lr-0.025-wd-0.0005-seed-1tl-max-120-wo12-B'
...
def margin_loss(logits,y,i=0,MT=True):
    ...
    T = torch.eye(logits.shape[1])[y.cpu()].to(device) * 999999
    TA = (logits-T)
...
def Adaptive_Auto_white_box_attack(logger, model, device, eps, is_random, batch_size, average_num, model_name='resnet18', data_set="cifar10",
                                   Lnorm='Linf'):
    ####  1.Loading datasets  ####
    if data_set=="mnist":
        num_class=10
    ...
    sorted_logits_index = np.zeros((ori_need_atk_num,logits_num),dtype=np.int32)
    natural_acc = 0
    begin_adv_acc = 0
    No_epoch = -2
    total_bn = 0
    ...
    logger.info("clean acc: {:.4f} ".format(clean_acc))
    ...
    for out_re in trange(out_restart_num):
        ...
        if out_re==2:
            ...
            # print(f" ##################### wrong_dire:{wrong_logits_direct:0.4} "
            #       f"correct_dire:{correct_logits_direct:0.4} ###################")
            # logger.info(" ##################### wrong_dire:{} \t correct_dire:{} ###################".format(wrong_logits_direct, correct_logits_direct))
        if out_re >2:
        ...
        for r in range(restart_num):
            ...
            # logger.info('No.{} clean_acc: {} robust acc: {} used_bp_num: {} used_fp_num: {} now_atk_num: {} now_correct_num: {}'
            #             .format(len(acc_curve), clean_acc, not_suc_index.sum()/len(all_atk_labs), used_backward_num, total_fn, now_need_atk_num, robust_acc_oneshot))
            acc_curve.append(f"{not_suc_index.sum()/len(all_atk_labs):0.4}")
            # print(acc_curve)
            # logger.info(acc_curve)
    logger.info('robust acc: {}'.format(acc_curve[-1]))
...
def main(random=True, average_number=100):
    logger = logging.getLogger(__name__)
    logfile = os.path.join(path, 'test_aaa.log')
    logging.basicConfig(
        format='[%(asctime)s] - %(message)s',
        datefmt='%Y/%m/%d %H:%M:%S',
        level=logging.INFO,
        filename=os.path.join(path, 'test_aaa.log'))
    ...
    model = ResNet18_FS(10).cuda()
    model.load_state_dict(torch.load(os.path.join(path, 'model-ema-20.pth')))

    model.to("cuda")
    model.eval()
    Adaptive_Auto_white_box_attack(logger, model, device, ep, random, batch_size, average_number, model_name='resnet18', data_set=data_set,Lnorm=Lnorm)
    etime = datetime.datetime.now()
    print(f'use time:{etime-stime}s')
    logger.info('use time: {} s'.format(etime-stime))

if __name__ == '__main__':
    main(average_number=1000)
